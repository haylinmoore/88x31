async function main(canvas){
    ...
    const magikoopa = await Sprite.create("./assets/magikoopa.png", 24, 31, 4, 3);
  	const magic = await Sprite.create("./assets/magic.png", 27, 24, 4, 4);
    ...
    ee31.attachLayerFunction('magikoopa', (ee31, frame) => {
        ...
        if (frame <= 7) {
            magikoopa.render(ctx, frame % 8);
        } else if (frame > 7 && frame <= 10) {
            magikoopa.render(ctx, 7);
        } else if (frame >= 11 && frame <= 19) {
            magikoopa.render(ctx, Math.floor((frame - 11) / 3) + 8);
        } else if (frame > 19 && frame < 60) {
            magikoopa.render(ctx, 10);
        } else if (frame >= 60 && frame < 64) {
            magikoopa.render(ctx, 7);
        } else if (frame >= 64 && frame <= 72) {
            magikoopa.render(ctx, 7 - (frame - 64));
        }
    };
